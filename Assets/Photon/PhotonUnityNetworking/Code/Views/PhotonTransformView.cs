...
namespace Photon.Pun
{
    ...
    public class PhotonTransformView : MonoBehaviourPun, IPunObservable
    {
        ...
        {
            var tr = transform;
            Debug.LogError($"stream.IsWriting:{stream.IsWriting} stream.IsReading{stream.IsReading},this.gameObject.name:{this.gameObject.name}");
            // Write
            if (stream.IsWriting)
            {
                if (this.m_SynchronizePosition)
                {
                    if (m_UseLocal)
                    {
                        ...
                        Debug.LogError($"IsWriting this.gameObject.name:{this.gameObject.name}tr.localPosition:{tr.localPosition}this.m_Direction:{this.m_Direction}");
                    }
                    else
                    {
            ...
            else
            {
                if (this.m_SynchronizePosition)
                {
                    ...
                    if (m_firstTake)
                    {
                        ...
                        this.m_Distance = 0f;
                        Debug.LogError($"m_firstTake Read this.gameObject.name:{this.gameObject.name}tr.m_NetworkPosition:{m_NetworkPosition}this.m_Direction:{this.m_Direction}");
                    }
                    else
                    {
                        ...
                        if (m_UseLocal)
                        {
                            ...
                            Debug.LogError($" Read this.gameObject.name:{this.gameObject.name}tr.m_NetworkPosition:{m_NetworkPosition}this.m_Direction:{this.m_Direction}");
                        }
                        else
                        {
